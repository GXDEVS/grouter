...
import { claudeChunkToOpenAI, newClaudeStreamState, translateClaudeNonStream, claudeToOpenAI, openaiToClaudeResponse, openaiChunkToClaude, newOpenAIToClaudeStreamState } from "./claude-translator.ts";
...
async function handleMessages(req: Request, pinnedProvider?: string): Promise<Response> {
  const start = Date.now();
  let anthropicBody: Record<string, unknown>;
  try { anthropicBody = await req.json() as Record<string, unknown>; }
  catch { logReq("POST", "/v1/messages", 400, Date.now() - start); return jsonResponse({ error: { message: "Invalid JSON body" } }, 400); }

  const authHeader = req.headers.get("Authorization");
  const requireAuth = getSetting("require_client_auth") === "true";
  let clientKey = null;
  if (authHeader?.startsWith("Bearer ")) clientKey = getClientKey(authHeader.slice(7).trim());
  if (requireAuth && !clientKey) {
    logReq("POST", "/v1/messages", 401, Date.now() - start);
    return jsonResponse({ error: { message: "Unauthorized. Invalid or missing Client API Key.", type: "invalid_request_error", code: 401 } }, 401);
  }

  // Translate ingress: Anthropic Messages → OpenAI chat.completions
  const body = claudeToOpenAI(anthropicBody);
  ...
  if (!provider) {
    logReq("POST", "/v1/messages", 400, Date.now() - start, { model: rawModel });
    return jsonResponse({
      error: {
        message: `Invalid model format: "${rawModel ?? ""}". Use "provider/model" (e.g. "anthropic/claude-sonnet-4-6") or send the request to a provider-specific port.`,
        type: "grouter_error", code: 400,
      },
    }, 400);
  }
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (!selected) {
      logReq("POST", "/v1/messages", 503, Date.now() - start, { model: rawModel });
      return jsonResponse({ error: { message: `No connections available for provider "${provider}"`, type: "grouter_error", code: 503 } }, 503);
    }
    if (isRateLimitedResult(selected)) {
      logReq("POST", "/v1/messages", 429, Date.now() - start, { model: rawModel, rotated: rotations });
      return jsonResponse(
        { error: { message: `All accounts rate limited. ${selected.retryAfterHuman}`, type: "grouter_error", code: 429 } },
        429, { "Retry-After": selected.retryAfter },
      );
    ...
    if (isTemporarilyUnavailableResult(selected)) {
      logReq("POST", "/v1/messages", 503, Date.now() - start, { model: rawModel, rotated: rotations });
      return jsonResponse(
        { error: { message: `All accounts temporarily unavailable. ${selected.retryAfterHuman}`, type: "grouter_error", code: 503 } },
        503, { "Retry-After": selected.retryAfter },
      );
    ...
    const label = selected.email?.split("@")[0] ?? selected.display_name ?? selected.id.slice(0, 8);
    const account = selected.auth_type === "oauth" ? await checkAndRefreshAccount(selected) : selected;

    const dispatch = buildUpstream({ account, body: normalizedBody, stream });
    if (dispatch.kind === "unsupported") {
      logReq("POST", "/v1/messages", 501, Date.now() - start, { model: rawModel, account: label, rotated: rotations });
      return jsonResponse({ error: { message: dispatch.reason, type: "provider_not_supported", code: 501, provider } }, 501);
    }
    ...
    let upstreamResp: Response;
    try {
      upstreamResp = await fetch(dispatch.req.url, fetchOptions as RequestInit);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      lastFetchError = { provider, url: dispatch.req.url, message: msg };
      console.log(`  ${chalk.red("✖")} fetch failed → ${chalk.cyan(label)} ${chalk.gray(dispatch.req.url)} ${chalk.red(msg)}`);
      excludeIds.add(selected.id); rotations++;
      markAccountUnavailable(selected.id, 503, msg, model || null);
      continue;
    }
    ...
    if (!upstreamResp.ok) {
      ...
      logReq("POST", "/v1/messages", upstreamResp.status, Date.now() - start, { model: rawModel, account: label, rotated: rotations });
      const ct = upstreamResp.headers.get("content-type") ?? "";
      if (ct.includes("json")) { try { return jsonResponse(JSON.parse(errText), upstreamResp.status); } catch {/* fall */} }
      return new Response(errText, { status: upstreamResp.status, headers: { "Content-Type": ct || "text/plain", ...corsHeaders() } });
    }
    ...
    if (stream) {
      ...
      const geminiState = fmt === "gemini" ? newGeminiStreamState() : null;
      const codexState = fmt === "codex" ? newCodexStreamState() : null;
      const anthropicState = newOpenAIToClaudeStreamState();
      let lineBuf = "";
      let tail = "";
      ...
      const { readable, writable } = new TransformStream<Uint8Array, Uint8Array>({
        transform(chunk, ctrl) {
          ...
          for (const line of lines) {
            ...
            // Stage 1: translate upstream format → OpenAI SSE (if needed)
            let openaiLines: string[];
            if (claudeState) openaiLines = claudeChunkToOpenAI(trimmed, claudeState);
            else if (geminiState) openaiLines = geminiChunkToOpenAI(trimmed, geminiState);
            else if (codexState) openaiLines = codexChunkToOpenAI(trimmed, codexState);
            else openaiLines = [trimmed + "\n"];

            // Stage 2: translate OpenAI SSE → Anthropic SSE
            for (const oaiLine of openaiLines) {
              for (const part of oaiLine.split("\n")) {
                const t = part.trim();
                if (!t) continue;
                const out = openaiChunkToClaude(t, anthropicState);
                for (const ev of out) {
                  ctrl.enqueue(enc.encode(ev));
                  tail += ev;
                }
              }
            }
        ...
        flush() {
          ...
          const usage = stateUsage
            ? { prompt: (stateUsage.prompt_tokens as number) ?? 0, completion: (stateUsage.completion_tokens as number) ?? 0, total: (stateUsage.total_tokens as number) ?? 0 }
            : (anthropicState.usage
              ? { prompt: anthropicState.usage.prompt_tokens ?? 0, completion: anthropicState.usage.completion_tokens ?? 0, total: (anthropicState.usage.prompt_tokens ?? 0) + (anthropicState.usage.completion_tokens ?? 0) }
              : null);
          logReq("POST", "/v1/messages", 200, ms, { model: rawModel, account: label, rotated: rotations, tokens: usage?.total || undefined });
          if (usage) {
            recordUsage({ account_id: selected.id, model: rawModel ?? "", prompt_tokens: usage.prompt, completion_tokens: usage.completion, total_tokens: usage.total });
            if (clientKey) updateClientKeyUsage(clientKey.api_key, usage.total);
          }
    ...
    // Non-stream: translate upstream → OpenAI → Anthropic Message
    let data = (await upstreamResp.json()) as Record<string, unknown>;
    if (dispatch.format === "claude") data = translateClaudeNonStream(data);
    else if (dispatch.format === "gemini") data = translateGeminiNonStream(data);
    else if (dispatch.format === "codex") data = translateCodexNonStream(data);
    const anthropicResponse = openaiToClaudeResponse(data);
    ...
    logReq("POST", "/v1/messages", 200, Date.now() - start, { model: rawModel, account: label, rotated: rotations, tokens: totalTok || undefined });
    return jsonResponse(anthropicResponse);
  }

  logReq("POST", "/v1/messages", 502, Date.now() - start, { model: rawModel, rotated: rotations });
  if (lastFetchError) {
    return jsonResponse({
      error: {
        message: `Upstream fetch failed for ${lastFetchError.provider} (${lastFetchError.url}): ${lastFetchError.message}`,
        type: "upstream_unreachable", code: 502, provider: lastFetchError.provider,
      },
    }, 502);
  }
  return jsonResponse({ error: { message: "All retry attempts exhausted", type: "grouter_error", code: 503 } }, 503);
